...
# 定义目标函数和偏导数
def Goal_func(x1, x2):
    value = 1 * math.pow(x1, 2) + 2 * math.pow(x2, 2) - 2 * x1 * x2 - 4 * x1
    return value
...
# Beta = -(Partial[1] * Search[0] + Partial[1] * Search[1]) / ((Search[0]  + Search[1] ) * Search[0] +(Search[0] + Search[1] ) * Search[1])
...
# 尝试绘制步进图
# s = np.arange(-15.0, 15.0,0.01)
# x1, x2 = np.meshgrid(s, s)
# fx = 1.5*(x1*x1) - x1*x2 + 0.5*(x2*x2) - 2*x1
# plt.contour(x1, x2, fx)
# C = plt.contour(x1, x2, fx)
